...
## 2026-08-12 · codex · eda — cache and strip production releases

- **good:** The whole-tree 71-check gate kept a release-critical optimization narrow: `pub-api-surface` isolated exactly the two intentional runtime build-ID functions and one executable-root capability, while every budget/style check stayed at baseline. Exact-commit preparation then passed 2,551/2,551 Debug tests and produced the stripped ReleaseSafe candidate in 231 seconds; a docs-only follow-up reused a byte-identical executable in one second and completed the full gate in four seconds.
- **good:** The unfiltered release runner caught a real regression introduced by the immediately preceding native-arc change: two dangling nested optional `if` expressions emitted straight-track headers and layers without coordinates. Its single named failure out of 2,551 localized the bad sync fixture; the corrected test now covers both straight and native-arc copper.

## 2026-08-12 · codex · eda — diagonal decoupling bonds and short differential-pair fans

- **good:** `prepare-release.sh` caught the one stale assumption that focused feature tests missed: the existing “blocked bond” fixture only blocked the compact pad hookup, so the new continuous fallback legally routed around it and reduced two plane vias to one. Tightening the fixture into a board-spanning obstruction preserved both behaviors explicitly; the final exact-commit gate passed all 2,552 tests and the ReleaseSafe build in 219 seconds of concurrent wall time.
- **good:** The `file-size` ratchet on the 10k-line router rejected a nine-line duplicate of the existing direct-route fallback. Reusing `tryDirectDogleg` made the production change debt-neutral and ensured plane-carried decoupling bonds follow the same DRC-probed fallback as ordinary two-terminal nets.
- **friction:** Green filtered runs still print `failed command:` for the test-runner invocation despite exiting 0; this task’s successful `directLegs`, `bound bypass`, and blocked-bond runs all carried that contradictory line, forcing exit-code checks each time.
...
## 2026-08-12 · codex · zig_genetic_cascades — production deployment

- **good:** The exact-commit ReleaseFast build passed both 71-check gates before the service was restarted; the deployed server then health-checked successfully and loaded the corrected 7,699-state database.
- **friction:** The deployment script used the ambient Zig 0.17 binary even though the consumer pins 0.15.1, failing before installation. A compatibility worktree then inherited the repository's shared `.zig-cache` symlink and twice failed the template `UpdateSourceFiles` step with a missing cached `zt-compile`; an explicit isolated `--cache-dir` produced the release cleanly. Pinning the deploy toolchain and isolating its cache would remove both release-time traps.

## 2026-08-12 · codex · eda — design-level RF island roughing

- **good:** Guardian's hard `file-size` gate stopped `optimizer.zig` at 10,011 code lines and pushed the ordered-path extractor, RF ranker, placement rougher, pose snapshot, edge orientation, and saved-outline alignment into cycle-free leaf modules; `cognitive-complexity` then caught the first cross-corner spoke legalizer and led to two small geometry helpers. The resulting boundaries are substantially easier to test than another monolithic optimizer pass.
- **good:** `pub-api-surface` made the new topology/ranking contracts and later callback/containment changes explicit, while `test-reachability`, `doc-comments`, `anytype-budget`, `bool-ops-per-condition`, and `unsafe-ops-budget` each caught an integration issue during iteration. The final whole-tree gate passed all 71 checks, and exact-commit preparation completed full tests plus the ReleaseSafe build in 227 seconds of concurrent wall time.
- **friction:** Green filtered test runs still print `failed command:` for the successful custom-runner invocation before returning exit 0. This task hit the contradictory line repeatedly while iterating on RF polarity, containment, outline, edge-row, and spoke-lane tests; naming it `runner command:` on success would remove needless exit-code rechecks.

## 2026-08-12 · codex · eda — legal courtyard edge contact

- **good:** After `main` advanced during release preparation, the rebased courtyard closer pushed `optimizer.zig` nine lines over its hard `file-size` cap. The gate forced the edge-contact predicate into the new courtyard leaf module and kept the optimizer at or below 10,000 code lines without accepting a ratchet; `pub-api-surface` then isolated the one intentional helper addition.
- **friction:** The sandboxed `guardian-check commit` run compiled the full 2,555-test suite, passed 2,515 tests, then reported 40 crashes that all came from HTTPZ mapping sandbox-denied `setsockopt(SO_RCVTIMEO)` to `catch unreachable`. Re-running unrestricted passed, but grouping identical syscall roots in the failure summary would make one environment capability failure look like one problem rather than forty regressions.
...
## 2026-08-12 · codex · zig_genetic_cascades — Zig 0.17 migration and review-first multipath deployment

- **good:** The root and server gates each passed all 71 checks after the compiler migration. `doc-comments`, `dead-pub`, `ban-globals`, and `catch-discipline` turned the new explicit-I/O compatibility layer from an undocumented shim with a swallowed `Io.Group.await` cancellation into a documented boundary with an explicit policy exemption and handled cancellation.
- **friction:** `guardian-check commit` upgraded every legacy v1 snapshot to v3 and installed a pre-commit hook whose baked binary pointed inside this consumer's ephemeral `.zig-cache/o/<hash>`. That expanded a roughly 50-file product/toolchain change to 169 committed paths and required a manual amend to restore the stable sibling Guardian path. Snapshot migration and hook installation should be explicit phases or preserve an already-managed stable hook target.
- **friction:** The first two `guardian-check commit` attempts passed all 71 checks but failed their test phase with `manifest_create ReadOnlyFileSystem` because Zig inherited a read-only global cache. Re-running with explicit writable global/local cache paths succeeded; echoing the effective cache paths in this failure would make the environmental cause immediate.

## 2026-08-12 · codex · eda — assembly layout-selection ZLS/rg A/B experiment

- **good:** Both independently developed arms were blocked on their first commit by actionable diff-scoped findings (unlinked SPEC coverage plus public-API/file-size growth), repaired those issues without accepting baselines, and then passed all 71 checks. The parent’s identical cold-cache `assembly` test runs and warm-cache `test-compile` runs also passed the gate with zero blockers.
- **friction:** Both successful filtered validation runs printed `failed command:` for the custom test-runner invocation even though 41/43 selected tests completed and the build exited 0. In a controlled comparison this made the visible result contradict the timing tool’s exit status in both arms; labeling a successful invocation `runner command:` would remove the ambiguity.

## 2026-08-12 · codex · eda — authored loop-filter routing before plane stitching
...
## 2026-08-12 · claude · eda — shared mask_relief module (fence-wide band + 1mm floor)
- good: one gated run surfaced 8 distinct blocking checks on the new module in a single report (file-size on the at-cap page file, completeness categories, 7-param function, missing doc comments, unguarded @intFromFloat, multi-loop tests, complexity creep) — each with the exact fix path; all 8 were resolved by real code changes rather than accepts, except pub-api (pure additions).
...
## 2026-08-13 · codex · eda — editable design rules and retained RF mask fillets
- **good:** `spec` caught a duplicate browser-contract tag, `bool-ops-per-condition` rejected an overgrown source-form predicate, and `pub-api-surface` made the new write endpoint an explicit review point. After those fixes, the commit-time and release-preparation gates completed all 71 checks with zero blockers.
- **friction:** Successful focused source-editor, browser-contract, Gerber, and settings-JSON tests again streamed `failed command:` for the custom runner even though every enclosing build exited 0, requiring separate status polling to establish each result.
- **good:** The exact-commit gate passed the full Debug suite in 81 seconds and built the ReleaseSafe candidate in 229 seconds; a temporary-project HTTP integration test also verified that the GUI save path preserved unrelated source and rebuilt successfully.

## 2026-08-13 · codex · eda — cache pour-derived route space
...
## 2026-08-13 · claude · eda — rotated-pad clearance keepout
- **good:** `spec` caught both halves of a sloppy first pass: a `// spec:` tag whose text had drifted from its SPEC.md bullet ("keeps its bounding box" vs "keeps its bounding box, which no rectangle can tighten") was reported as BOTH `unlinked tag` and `unverified`, which named the exact mismatch instead of leaving me to diff two strings by eye. Fixing the tag to match verbatim cleared both.
- **friction:** Successful focused `pad_shape` tests again streamed `failed command: cd . && ./.zig-cache/o/<hash>/test ...` while the enclosing build exited 0 — the same artifact logged on 2026-08-13 by codex four times over. It cost two extra confirmation runs (one with a changed `--seed` to bust the cache) before I trusted a green result, because a genuinely failing run looks nearly identical apart from an `error: '<test>' failed:` line further up. A non-zero runner exit or suppressing the line on success would remove a recurring "is this green?" round trip.
- **friction:** A cached green `zig build test` prints no `N pass` tally at all — the run step is skipped, so the only signal is the exit code. Combined with the `failed command:` line above, a cached success and a real failure differ by the presence of one line in a ~40-line stream. A one-line `guardian/test: cached, N test(s) previously passed` would make the cache hit self-evident.
- **good:** `guardian-check` is fast enough that the diff-scoped gate (71 checks, 1 file in scope) was never the thing I waited on; the full Debug suite ran 2538 tests in 65.10 s under `scripts/gate.sh`, and the cached re-run to capture the summary line came back in seconds.

## 2026-08-13 · codex · eda — field route-space net and trace quality
- **good:** `spec` caught a benchmark contract whose updated bullet no longer exactly matched its test tag, and `pub-api-surface` previewed only the four intentional candidate/finisher seams; the snapshot merge driver then reconciled two newer `main` additions during rebase without conflict.
- **good:** The final whole-tree gate completed all 71 checks with zero blockers, the full Debug suite passed in 84 seconds, the ReleaseSafe candidate built in 236 seconds, and the exact candidate deployed with green health probes.
- **friction:** Every successful focused test again streamed `failed command:` for the custom runner even though the enclosing build exited 0, requiring a separate final-status poll after each route-space, diagnostic, benchmark, and seed-gate filter.

## 2026-08-13 · claude · eda — canonical PCB layer table (Phase 0 refactor)
- **good:** `dead-pub` did exactly the right thing on a keystone refactor: a brand-new shared module (`src/board_layers.zig`) exports ~30 declarations, and the check named the six nobody consumed yet (`isCopper`, `isOuter`, `kicadName`, `fromRaw`, `max_sidecar_layer`, `layerTable`). Each one turned out to be a call site I had *meant* to convert and had skipped — so the check found four real gaps in the refactor, not style debt.
- **good:** `type-size` (Row at 8 fields, cap 7) and `unsafe-ops-budget` (one new `= undefined` struct-field default, repo budget 7) both pushed the new type into a better shape — grouping the Gerber suffix + X2 file-function into a nested `GerberId` and defaulting every `Row` field so the backing array is `@splat(.{})` instead of `undefined`. Two small, clearly-correct improvements from checks firing on a file that had no baseline at all.
- **friction:** `change-classification` is scoped to the **staged/working diff vs HEAD** in the pre-commit hook, but to the **branch diff vs the base ref** in a manual `guardian-check all .`. That difference silently makes an incremental commit sequence impossible for a refactor whose tests all live in the first commit: the second commit ("collapse 8 files onto the shared lookup, no new behaviour") reported 8 × "1 behavioral line(s) added" and demanded a test, even though the branch as a whole added 11 spec-tagged tests. `git commit --amend` did **not** clear it either (amend re-diffs against HEAD~1 = the first commit). Cost: three failed commit attempts plus a patch/restore dance, and ultimately a differently-shaped commit history than intended. A hint in the failure text — "scoped to this commit's diff; earlier commits on this branch are not counted" — would have saved all of it.
- **friction:** The same job's `dead-pub` interaction makes small commits structurally impossible for a *new shared module*: commit 1 (module only) fails `dead-pub` because consumers land in commit 2, and commit 2 fails `change-classification` because the tests landed in commit 1. There is no commit ordering that satisfies both. Worth considering whether `dead-pub` should tolerate a declaration referenced by a test in the same file (it does) *or* by an uncommitted working-tree change.
- **friction:** Successful focused tests again streamed `failed command: cd . && ./.zig-cache/o/<hash>/test ...` while the enclosing build exited 0 — now logged by three different agents across five entries on this one day. It cost two extra "is this actually green?" polls here too.
- **good:** `GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface zig build` handled a 62-declaration public-API addition in one selective refresh, and the resulting `.guardian/pub-api.txt` diff was legible enough to eyeball as intended. Final gate: 71 checks, 0 blocking; full Debug suite 66.21 s under `scripts/gate.sh`.

## 2026-08-13 · claude · eda — rotated courtyard + mask-dam keepouts
- **good:** `file-size` blocked a 16-line public helper I had parked in `src/placement/optimizer.zig` (10016 code lines against the 10000 hard limit) and, unlike the ratcheted per-item checks, said plainly that the cap was hard rather than offering an accept. That pushed the function into `pad_shape.zig` beside the courtyard helper it belongs with — a better home than the one I picked, arrived at in one gate cycle.
- **good:** `pub-api-surface --verbose` classified the diff as "3 new symbol(s), 0 changed, 0 removed — pure additions, safe to accept". That one line is what made accepting the snapshot a decision rather than a shrug; a changed or removed signature in the same list would have read completely differently.
- **friction:** `test-no-conditional` reported "more than one top-level loop — the loop at line 2534 asserts nothing; extract that one into a fixture helper", which named the right line but not the rule: I had to infer that the objection is a SECOND top-level loop that only computes, not the `if` inside the first one. The fix (hoist the computing loop into a named helper) took one guess. Naming the rule in the finding ("a test may have one top-level loop; loops that only compute belong in a helper") would remove the guess.
- **friction:** `guardian-check pub-api-surface . --verbose` is the working spelling; `guardian-check run pub-api-surface .` prints the whole command list with no hint that `run` is not a verb here. A one-line "unknown command 'run' — check names are used directly" would have saved a round trip.
- **good:** The whole-tree gate stayed cached across a commit whose inputs had not changed since the last green run ("run-all: cached — 0 blocking"), so the commit itself was instant after a 2694-test suite had already passed.

## 2026-08-13 · claude · eda — PCB layer quick-win bug fixes (6 small, independent fixes)
- **friction:** `file-size`'s frozen per-item ratchet turned a 70-line bug fix into a 90-minute accounting exercise. Two of the files a PCB-layer fix must touch sit ON their ceilings: `src/placement/optimizer.zig` is at 9999 of a 10000 HARD limit (a 15-line documented method put it at 10014), and `src/serve/pcb_layout_page.zig` was ratcheted at exactly 10512, of which my earlier commits on the same branch had already spent the last 1 line of headroom. Net effect: the natural home for a shared layer-colour rule was unusable, and the validation helpers had to be relocated to `layout_sidecar_json.zig` mid-task. Not arguing the check is wrong — the files ARE too big — but a fix landing in a maxed-out file has no incremental path, and the ceiling is invisible until you've already written the code. A `guardian-check headroom <file>` (or printing "N lines of headroom remain" on a passing run of a near-ceiling file) would let an agent choose the right home BEFORE writing.
- **bug (or at least a costly surprise):** `file-size`'s reported count does not move when you delete lines from a `test` block — I removed 7 lines from a test and the count stayed at 10518 (verified live: injecting 7 non-test lines moved it to 10525, so the counter is not cached). The finding text ("10518 code lines", "68 code lines over its frozen ratchet ceiling") gives no hint that test bodies are excluded, so I spent three edit/measure cycles compacting tests that were never counted. Saying "code lines (test blocks excluded)" once, in the finding, would have made the first cycle the last one.
- **good:** `pub-api-surface` again earned its keep: three separate additions across the task (`render_pcb_png.trackColor`, then `layout_sidecar_json.stackupLayerRules`/`saveRejection`) each came back as "N new symbol(s), 0 changed, 0 removed — pure additions, safe to accept", which made `GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface zig build` a decision rather than a rubber stamp.
- **good:** The counting test runner caught a genuinely dangerous mistake: after moving helpers + their test into `src/serve/layout_sidecar_json.zig`, the filtered run reported "0 match by name" and FAILED with "NOTHING YOU ASKED FOR RAN". That module was not in `src/main.zig`'s test aggregator, so the moved test would otherwise have been silently dead. Exactly the failure mode the empty-filter gate exists for, hit for real.
- **friction:** Successful focused tests once more streamed `failed command: cd . && ./.zig-cache/o/<hash>/test ...` while the build exited 0 — this is now four agents across six entries on the same day. It cost roughly eight extra "is this actually green?" polls over six fixes (I ended up running the cached test binary by hand each time to read "All N tests passed"). Whatever the cause, the string "failed command" on a green run is the single most expensive false signal in this repo right now.
- **good:** Final whole-tree gate: 71 checks, 0 blocking, 2692 tests green in 66.87 s under `scripts/gate.sh`, and the unchanged-tree rerun was a 1.4 s cached no-op.

## 2026-08-13 · claude · eda — PCB layer wire-format standardization (Phase 1)
- **good:** The counting test runner caught a second dead-test module, the same way it caught one yesterday: `src/serve/pour_json.zig` had three `// spec:`-tagged tests that had NEVER run because the file was missing from `src/main.zig`'s aggregator, and my new test there came back "0 match by name — NOTHING YOU ASKED FOR RAN". Two independent finds in two days suggests the aggregator gap is systemic in this repo; a check that lists `src/**.zig` files containing `test "` but absent from the aggregator's import block would find the rest in one pass instead of one accident at a time.
- **friction:** `file-size` again, and again on `src/serve/pcb_layout_page.zig` (ratcheted at 10512). This task legitimately had to touch that file — the sidecar types, the blob writer and the MCP `add_tracks` handler all live there — and the ceiling turned ~100 added lines into five successive shrink-and-remeasure cycles (10585 → 10572 → 10549 → 10530 → 10516 → under). Each cycle is a full gate run to learn the new number. The outcome was genuinely better (a new `src/serve/layout_layers.zig` module), but I only discovered the right size budget by binary-searching against the checker. Echoing yesterday's request: print remaining headroom on a PASSING run of a file within ~100 lines of its ceiling, so the budget is known before the code is written rather than after.
- **good:** `type-size` fired on three different structs during this task and was right twice and usefully wrong once. Right: `SolvedRequest` at 9 fields pushed `restored_routes` + the new layer audit into one `Restored` value, which is a genuine cohesion win (a caller can no longer hold copper without the verdict on that copper). Usefully wrong: `SavedVia` (8) and `drc.Violation` (8) are both records that mirror a fixed external shape — a JSON object and a report row — where nesting would only churn every construction site to hide the field count; `guardian-check accept type-size .` was the right answer, and the fix text offering both options made that an explicit decision rather than a dodge.
- **good:** Accepting the `type-size` snapshot also ratcheted two UNRELATED items DOWN (`geometry.Geom` dropped out entirely, `optimizer.Part` 18 → 17) because earlier work had shrunk them. A ratchet that tightens on every accept means the accept is not purely a loosening, which made it much easier to justify.
- **friction:** `spec` correctly flagged a bullet I had orphaned by renaming its test, but the finding ("unverified: Web Server - <bullet text>") does not say which test USED to carry that tag or that the tag exists nowhere in the tree. On a rename the useful message is "no test carries this tag" vs "this tag is on a test in a file the aggregator does not import" — those are opposite fixes, and I guessed.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test ...` on green runs, once more — five agents, seven entries, two days. It cost about six extra confirmation polls across this task; I ended up scripting `echo "exit=$?"` after every single filtered run because the streamed text is unreliable. If the cause is the custom runner's exit path, it is worth prioritizing: it makes every other diagnostic in the stream less trustworthy.
- **good:** Final whole-tree gate: 71 checks, 0 blocking, 2702 tests green in 66.73 s under `scripts/gate.sh`.

## 2026-08-13 · claude · eda — PCB layer fab/export correctness (Phase 5: Gerber + Excellon)
- **good:** `type-size` shaped two designs for the better in one task. `board_layers.Row` went to 9 fields when it gained a board `side` and a second (UI) layer name; rather than accept, grouping the naming facts into a `Naming { kicad, ui, color }` value took it back to 7 and made "what this layer is called" one thing instead of three loose fields. `geometry.Pad` was the sharper case: it sat exactly ON its frozen ceiling of 13, and the two fields a fab bug fix genuinely needed (`mask_margin`, `no_paste`) would have made it 15. The forced regrouping produced `Pad.Overrides { rratio, mask_margin, no_paste }` — the three `(pad …)` declarations that each override a project-wide default — which is a real concept the struct was missing. Both times the check refused a lazy field and got a better type.
- **friction:** …but finding WHICH fields to group was pure guesswork against the cost of a `git grep`. `Pad` needed to shed one existing field to make room, and the only way to choose was to count call sites by hand for each candidate (`rratio` 12, `slot_half` 10, `npth` 19, `shape` 34, `drill` 37, `poly` 51) and pick the cheapest that was not semantic violence. A `guardian-check explain type-size --item src/placement/geometry.zig::Pad` that listed each field with its read-site count would turn a 20-minute survey into one command. Same shape of request as the `file-size` headroom asks in the two entries above: the checks know the item is over, but the agent has to reconstruct the budget from scratch.
- **good:** `function-size`'s param cap (6 runtime) fired on `excellonDrill` reaching 7 and was right for the right reason: the two arguments I was about to pass separately — the drill class and the board's copper count — are exactly the two things the file's `TF.FileFunction` attribute states, so they wanted to be one `DrillFile` value. Same story a commit earlier, where `writeLayer` would have hit 9 params and instead grew a `Meta { function, created }`. Twice in one task the param cap named a missing type rather than an inconvenience.
- **good:** `ban-time` caught `std.time.timestamp()` in a serve handler within seconds of me writing it, and the fix text ("inject a Clock port from infra/clock") named the exact module — no searching. This one is a pure win: the file already imported `infra/clock`, and I had simply reached for std out of habit.
- **friction:** `test-no-conditional` again reported "more than one top-level loop — the loop at line 2247 asserts nothing; extract that one into a fixture helper" without naming the rule, which is the same friction logged in the 2026-08-13 courtyard entry. It happened to point at a loop that SHOULD have been production code (counting copper files for the job file's LayerNumber, which the writer needed too), so the extraction improved the source — but that was luck, not guidance. Restating the rule in the finding would make the good outcome reproducible.
- **good:** `spec`'s deny_growth kept six behavior changes honest across six commits — each new SPEC bullet had to land with its tagged test in the same change, and renaming an existing bullet (the "Inner"→"Inr" file-function change) forced me to update the test tag in lockstep, so the spec cannot drift from what the tests actually assert. For a task whose whole point was DELIBERATE byte-level output changes, that coupling is exactly the right pressure.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test ...` on green runs, still — six agents, eight entries, two days. I gave up reading the stream on the second run and wrapped every single invocation as `zig build … > log 2>&1; echo "EXIT=$?"`, which is now the only way I trust a filtered run in this repo. Cost across this task: ~10 wrapped invocations that should have been plain ones.
- **good:** Final whole-tree gate: 71 checks, 0 blocking, 2705 tests green in 65.72 s under `scripts/gate.sh`, and an explicit `guardian-check all . --gate --full` on the final tree agreed.

## 2026-08-13 · codex · eda — JLC-safe Gerber export
- **good:** `spec` caught the new RF fab-readiness regression test before it had a matching `SPEC.md` contract, and `file-size` prevented adding another helper to the already-ratcheted `pcb_layout_page.zig`; moving JLC filename policy into `serve/fab_filename.zig` made the boundary clearer.
- **friction:** A test in the new module initially produced the useful zero-match failure because `src/main.zig` did not aggregate it, but the successful focused runs still printed `failed command:` while exiting 0, requiring final-status checks to distinguish green from red.
- **good:** The final whole-tree gate passed all 72 checks with zero blockers; full Debug tests passed in 83 seconds, the ReleaseSafe candidate built in 239 seconds, and the exact candidate deployed with green health probes.
## 2026-08-13 · claude · eda — turn on the new `concept` check (declare owned concepts, freeze existing drift)
- **good:** `concept` paid for itself on its first run. It found a TENTH power-rail classifier that the hand audit had missed: `src/serve/assets/editor.js` carries its own `isGroundName` / `isPowerName` regex pair beside the nine Zig ones. The audit missed it because `grep -rl` over `src/` silently skips that file — it contains six deliberate NUL bytes (composite map keys, `pad + "\0" + net`), so GNU grep classifies it as binary and reports no match at all (`grep -c AVDD editor.js` → exit 1; `grep -ac` → 1). A cross-language lexical scanner reading bytes rather than lines is exactly the advertised value, and it beat the tool the audit was built with.
- **good:** the violation text is the most actionable in the suite: file:line, concept name, occurrence count with line numbers, the first spelling found, the owner path, and the rule's own `reason`. I never needed `explain` after seeing one.
- **good:** `explain concept` was accurate on every mechanical question I checked before writing declarations — per-(file,concept) identity, dot-directory skipping, `*` semantics, guardian.toml self-exemption. Its "a project may declare the concept before the owner exists" clause is load-bearing here: our owner module `src/board_layers.zig` only exists on an unmerged branch, and declaring against a not-yet-existing owner behaves exactly as documented.
- **bug:** `files` is documented per rule ("optional scan set") but implemented as a shared union. In `checks/concept.zig`, every rule carrying a `files` key lands in ONE `ScanCtx`; `globbed()` returns true when ANY rule's glob matches a path, and `scan()` then calls `analyzeFile(..., self.rules)` — i.e. every glob rule is applied to every file the union names. Repro: declare rule A `files = ["src/*.js"]` beside rule B `files = ["src/*.zig", "src/*.js"]`; A reports Zig offenders. Cost: the sharpest target of our audit is undeclarable. The 27 DRC kind ids are hand-copied into `src/serve/assets/pcb_settings.js` (DRC_GROUPS / DRC_HELP), so renaming a kind in Zig silently drops its label, group and help text from the settings drawer — but their bare spellings are legitimate typed enum use (`.kind = .track_pad`) in 49 Zig files. Scoping the rule to `src/*.js` scores exactly 4 offenders and is the whole fix; the union makes it 73. A per-rule scan set (or a per-rule `exclude`) lands it unchanged.
- **bug:** a literal cannot contain a double quote, and the failure is silent. `literals = ["\"track_track\""]` parses with the backslash intact and matches nothing — reported as `ok: 0 violation(s)`, indistinguishable from a clean tree; `literals = ['"track_track"']` is rejected outright with `invalid value for 'literals'`. Quoting is the natural discriminator between "this file names the id as data" and "this file uses the enum": measured on the same vocabulary, 13 files quoted vs 49 unquoted. Honoring `\"` or accepting TOML literal strings would make several otherwise-undeclarable families declarable. The silent half is the worse half — a rule that can never match looks exactly like a rule with nothing to find.
- **wish:** a preview mode that prints a proposed rule's offender list without touching `.guardian/`. Tuning eight literal sets meant run → `accept` (to get a baseline file readable enough to see WHICH files) → delete it → edit → repeat, six or seven times. With no baseline present, `guardian-check concept . --verbose` prints only `N violation(s) grandfathered`; printing the rows there would have removed the accept/delete dance entirely and is the single change that would most speed up adopting this check.
- **good:** baseline + `deny_growth` mechanics did exactly what the docs promise, verified deliberately rather than assumed. A new offender FILE fails (exit 1). A new (file, concept) PAIR inside an already-frozen file fails — so freezing is per pair, not per file, which is finer than I expected and better. An extra occurrence of an already-frozen pair passes, as documented. And `accept` with "concept" in `deny_growth` refuses: "acceptance refused because the configured deny_growth policy would grow recorded debt", then restores the pre-run snapshots.
- **friction:** minor — `guardian-check accept <check> .` runs the whole-tree check three times (three `run-all: 1 check(s) passed` lines for one accept). Free for `concept`; it would be painful for a check that costs real seconds.
- **good:** final whole-tree gate: 72 checks, 0 blocking, full suite green in 66.39 s test wall under `scripts/gate.sh`, with the new check reporting `concept: baseline matches (139 violation(s))`.

## 2026-08-13 · claude · eda — PCB Appearance panel rebuild (Phase 2)
- **good:** The whole-tree gate earned its keep on this one. The task rewrote ~400 lines of an `@embedFile`'d browser asset, and Zig tests assert on that asset by string content — so `zig build test-compile` was green, six different `-Dtest-filter` runs were green (110 tests), and the full gate still found a real failure: `drc_rules.zig`'s "viewer JS keeps placement guides independent with inspection defaults" asserted a defaults literal I had reshaped. Nothing cheaper could have found it, because the stale marker lived in a test whose NAME shared no substring with anything I had changed. Worth remembering the next time someone argues the gate is redundant after `test-compile`.
- **wish:** …and that is the shape of a check Guardian could own. When a `@embedFile`'d asset changes, the tests that assert on its CONTENT are exactly the ones at risk, and they are mechanically identifiable: `std.mem.indexOf(u8, <name-bound-to-that-embed>, "literal")`. I wrote a throwaway Python sweep to find the rest and it produced ~55 false positives, because in this repo several files bind the identifier `js` to a DIFFERENT embed in each test scope, and a line-based scan cannot tell them apart — Guardian already has the AST index that could. Something like `guardian-check explain asset-assertions --file src/serve/assets/pcb_board.js` listing every test literal asserted against that asset, with present/absent status, would turn a gate cycle (~10 min) into one command.
- **friction:** `file-size` on `src/serve/pcb_layout_page.zig` again (frozen at 10512) — third or fourth entry on this exact item. This task only needed **+1 line** there (a third Appearance tab), but I could not tell whether that was affordable, because the finding's number and the file's actual line count disagree wildly: `wc -l` says 13485, the baseline says 10512. I had to write a Python script that strips `test "…" { … }` blocks to reverse-engineer that Guardian counts non-test lines (10510 — two lines of headroom), and only then knew I had to offset my +1 by shrinking a neighbouring writer. The metric's DEFINITION is the missing piece, not just the headroom: `explain file-size` describes the check but never says what it counts, so an agent budgeting against it is guessing. Printing "src/serve/pcb_layout_page.zig: 10510/10512 non-test lines" on a passing run would have saved the whole detour.
- **good:** `test-no-conditional`'s "more than one top-level loop" fired on two new tests where I had a positive-marker loop plus a negative-marker loop, and the fix — one `Check{marker, present}` table with a single loop — is both the house pattern in this file and objectively better: the two loops had drifted into asserting with different phrasing, and merging them made the "these strings must be gone" cases sit next to the ones that replaced them. Second time this month the check has improved a test rather than merely nagged.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test …` on green runs, again — I lost a full round trip to it early on: the build printed `+- run test w` (truncated) and `failed command:`, so I ran the test binary by hand to discover "All 22 tests passed", then checked `$?` and found the build had exited 0 all along. Every subsequent invocation in this session was wrapped as `zig build … > log 2>&1; echo EXIT=$?`. That is now at least the ninth entry reporting this; it is cheap to work around but it makes the whole build stream untrustworthy, which costs more than the workaround.
- **good:** `spec` + `deny_growth` handled a bullet DELETION cleanly, which I had not tested before. This phase deliberately contradicted an existing bullet ("The Objects tab contains only the selection filter; clearance halos live in the Layers tab…") — removing the bullet and its tagged test in the same commit as the behavior change passed without argument, and adding six replacement bullets each required its tagged test in that same commit. Exactly the right pressure for a UI restructure, where it is otherwise very easy to leave the spec describing last month's panel.
- **good:** Final whole-tree gate: 72 checks, 0 blocking, 2713 tests green in 66.45 s under `scripts/gate.sh`.

## 2026-08-13 · claude · eda — one PCB colour theme, consumed by every renderer (Phase 3)
- **friction:** `naming` blocks Zig's ordinary NAMESPACE idiom. A palette of related constants written as `pub const review = struct { pub const mask = "#086b43"; … };` fails with "pub const review is a struct_ type but is not PascalCase — fix: PascalCase iff the fn returns `type`; types use PascalCase." But this is not a type anyone instantiates; it is a namespace, and `Review.mask` would read as a struct field access on a value that never exists. std itself does this (`std.math`, `std.ascii` are namespace structs, lowercase). I flattened twelve constants to a `review_` prefix to pass, which is strictly worse than the grouping. Suggestion: exempt a struct with no fields and no `pub fn` returning `type` — i.e. a pure constant namespace — or key the rule on whether the decl is ever used as a type.
- **friction:** `file-size` on `src/serve/pcb_layout_page.zig` (frozen 10512) — third entry today, same file, same confusion, so treat this as a vote rather than a new report: the finding prints a number `wc -l` disagrees with (13485 vs 10512) and never says the metric is non-test lines. The concrete cost here: my change needed **+3** lines in that file (one import, one blob call, one CSS call), and I could not tell whether 3 was affordable, so I first wrote the code inline, gated, got blocked at 10567, and then had to re-plan. Printing "10510/10512 non-test lines" on a PASSING run would let an agent budget before writing rather than after.
- **good:** and that block was right. The fix was to move the new emitters into the module that owns the data (`board_theme.zig` writes its own JSON and its own CSS custom properties) and the new asset-contract test into `serve/static_assets.zig` where the house pattern already lives, then pay for the remaining +3 by deleting a single-use alias and folding a one-statement `if` in the function I was already editing. Net zero lines in the big file, three genuinely better placements. `explain file-size`'s "Split along a cohesive module seam" was the correct advice.
- **good:** `dead-pub` caught something real and cheap. Three theme constants (`copper_inner`, `blame_low`, `blame_high`) existed only because I had transcribed a palette wholesale; two became used when the PNG renderer was converted, and the third genuinely had no consumer, which made me write the assertion that the layer table's inner rows ARE that palette — a real cross-check I would not otherwise have written. A "you declared this and nobody wants it" nudge is exactly the right pressure on a constants module.
- **good:** `test-no-conditional`'s "more than one top-level loop" fired twice and improved both tests — once into `expectPalette(entries, styled)` shared by the board and review palettes, once into `expectContainsAll(haystack, needles)` which the asset file wanted anyway. The finding names the loop, so the fix is mechanical.
- **friction:** `pub-api-surface` on a NEW module is 49 findings in one go (every `pub const` colour), truncated to "+46 more — use --verbose". That is correct behaviour and the accept flow worked first try (`GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface`), but the check name and the env-var name differ (`pub-api-surface` vs the `.guardian/pub-api.txt` file, and the docs' `GUARDIAN_UPDATE_SNAPSHOT=pub-api`) — I guessed `pub-api` first from CLAUDE.md and it silently accepted nothing. The failure output does print the right spelling; worth making the two names the same.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test … --listen=-` on a GREEN run, again (ninth-plus report). Exact cost this session: two extra full gate cycles (~9 min) — I saw the banner on a `tail -25`, could not tell whether tests had failed, re-ran the whole suite to a log file, found exit 0, then re-ran a THIRD time with a fresh `--seed` because the second run was a cached no-op and I could not distinguish "cached success" from "cached borrow". Only `--summary all` (19/19 steps succeeded) settled it. The banner makes a passing build unreadable without a second command.
- **good:** whole-tree gate: 72 checks, 0 blocking, 2713 tests green in 66.45 s under `scripts/gate.sh`; `zig build test-compile` clean; the `external` node-syntax gate on `pcb_board.js` caught nothing because I ran `node --check` myself after every asset edit, which is what that gate trained me to do.

## 2026-08-13 · claude · eda — turn on the relational checks that needed no Guardian code (Wave A)
- **friction:** a brand-new `[[ban]]`/`[[boundary]]` rule cannot be verified until you first materialize its baseline file, and the failure mode is silence. With no `.guardian/baselines/ban.txt` on disk, my deliberate probe (a module-qualified `page.writeLayoutsFileJsonRev(...)` plus a bare private-fn call, both in a file outside the allow list) reported `ok: ban: 2 violation(s) grandfathered` and exited **0** — i.e. the check found exactly what I wanted it to find and then froze it. That is baseline mode working as designed, but it means "I wrote a rule, does it block?" reads as green on the one run where the answer matters most. Cost: one wasted probe cycle plus the detour to work out why. Fix that would have saved it: when a baseline is being CREATED (not matched), say so in those words — `ban: no baseline existed; recording 2 violation(s) as the starting set` — instead of `ok: … grandfathered`, or offer `--dry-run` that prints offenders and writes nothing. The workaround, for the next agent: run `accept <check> .` on the clean tree first to write the empty baseline, THEN probe.
- **good:** once a baseline existed, both probes were exactly the diagnostic you want. `boundaries` with a temporary `@import("../serve/sync.zig")` in `src/kicad_pcb/project_rules.zig` failed with `1 new violation(s) above baseline of 66` and named the rule that broke (`rule: src/kicad_pcb/*`), and `ban` printed `<file>:<line>: writeLayoutsFile is banned here — <reason>` with my configured reason verbatim. Nothing had to be guessed from the message.
- **wish:** document that a length-1 `[[ban]]` chain is alias-proof, because the current doc implies the opposite. `BanRule`'s comment says `["optimizer","placeFromPoses"]` bans `optimizer.placeFromPoses`, which reads as "spell the module prefix" — and that spelling is the WEAK one, since the matcher does no alias resolution and an importer may bind the module to any name. Reading `banned_symbol_helper.Match.onIdent` shows a single-token chain completes on the identifier regardless of what precedes it, so `["writeLayoutsFileJsonRev"]` catches the call under every alias AND as a bare same-file call. I only used the strong form because I read the state machine; an agent trusting the doc comment would ship the weak rule. One sentence on `BanRule.chain` fixes it.
- **friction:** `[[ban]]` inherits `ScanOpts{ allow_in_tests = true, allow_in_main = true }` and there is no per-rule override, so a banned call inside a `test { … }` block or `pub fn main` is invisible. For the compiled hidden-dependency bans that default is obviously right; for a project rule that says "this write path must go through the guarded seam" it punches a hole I cannot close from guardian.toml — a new file's test helper may persist an unguarded sidecar and the gate is silent. A per-rule `in_tests = true|false` key would let a project choose.
- **wish:** `[[boundary]]` has no `allow` list, so a rule is all-or-nothing and every legitimate exception has to live as baseline debt. `[[ban]]` got `paths` + `allow` and it is the reason those rules read as intent ("these three files hold this call, on purpose") rather than as a frozen pile. Same two keys on `[[boundary]]` would let `src/placement/*` forbid the export layer while naming the two files that genuinely straddle it, instead of burying them among 63 baselined lines where nobody can tell deliberate from accidental.
- **good:** `[test_reachability] roots` did the whole job in one line, including the cleanup. This tree roots its test binary at `src/test_root.zig`, which is neither of the two names the check guesses when `roots` is unset and is not reachable from `main.zig` — so the check had recorded `src/test_root.zig` itself as debt. Declaring the real root made the entry resolve, the check printed `1 resolved (run accept to prune)`, and `accept` took the baseline to zero. Self-correcting, no manual baseline surgery.
- **good:** the whole point of the session worked — two relational rules that needed no Guardian code now hold ratchets on real architecture: `src/kicad_pcb/*` must not import `serve/` (3 sites frozen) and `src/placement/*` must not import the export layer (63 sites across 49 files frozen), plus five `[[ban]]` rules starting at zero debt. `forbidden` matching SUBSTRINGS of the RESOLVED import path is the right call — `../serve/sync.zig` normalizing to `src/serve/sync.zig` before comparison is what let one 8-character rule name the edge.
- **good:** final whole-tree gate: 72 checks, 0 blocking, full suite green, test wall 70.84 s under `scripts/gate.sh`.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test … --listen=-` printed on the GREEN run again (tenth-plus report; logging it as a vote, not a new finding). Zero cost this time only because every earlier entry trained me to wrap the invocation as `… 2>&1 | tail; echo EXIT=$?` from the start.

## 2026-08-13 · claude · eda — two MCP schema↔handler contract fixes (undeclared args + array-vs-comma-string types)
- **good:** `explain test-no-conditional` paid for itself BEFORE I wrote a line. I was about to write two guard tests each with a `for` over a table of tools plus behavioural asserts; the explain text ("extra `for` loops at the top level… the finding names the loop that asserts nothing") told me the shape to use instead, so both walks went into plain helpers (`firstUndeclaredOption`, `firstCommaTypeMismatch`) that return `"<tool>.<key>"` or `""` and the test bodies became one `expectEqualStrings`. That is strictly better than what I planned: the failure now NAMES the offending tool+argument (`get_layout_progress.rough`) instead of pointing at an anonymous `expect(false)` inside a loop. Reading `explain` first is cheaper than the retry it prevents — worth advertising that in the check's own output.
- **good:** `pub-api-surface` on ONE new `pub fn` was a clean, first-try accept. The failure printed the symbol with its full signature, the exact command (`GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface zig build`) AND a delta line — "1 new symbol(s), 0 changed, 0 removed — pure additions, safe to accept" — which is the sentence that let me accept without re-reading the snapshot. Contrast with an earlier entry today where the env-var spelling had to be guessed: when the failure text carries the exact incantation, the accept flow is a non-event.
- **good:** `spec` + `deny_growth` handled a bullet REWORD, not just an add/delete. I generalized an existing bullet ("every option **the describe read handler** honours is declared in **describe_pcb_layout's** strict input schema" → "every option **a PCB read handler** honours is declared in **that tool's** strict input schema") because the test behind it stopped being describe-only, and edited the `// spec:` tag in the same commit; the two new bullets each landed with their tagged test. No argument from the gate, and the pressure is right — the bullet would otherwise still describe a one-tool check.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test … --listen=-` on a GREEN run, again (eleventh-plus report — logging it as a vote). Cost this session: one round trip. My filtered run printed `guardian/test: 20 test(s) selected by filter … test wall 0.01s` followed immediately by `failed command:`, with no `error:` anywhere, so I ran the cached test binary by hand to get `All 20 tests passed` before I trusted it. Every later invocation was wrapped as `… 2>&1 | tail; echo EXIT=${PIPESTATUS[0]}`. Note the banner also appears when the build genuinely fails for an unrelated reason (a blocking Guardian check), so it is not even a reliable "ignore me" marker — that ambiguity is the expensive part.
- **wish:** a way to assert "this checked list is exhaustive". Both defects here were one side of a contract drifting from the other (an MCP tool's JSON Schema vs. the handler that reads the arguments), and my fix is a table of the three tools that route through a shared options builder, guarded by a comment saying "adding a caller means adding a row here". Guardian already knows how to find call sites — `dead-pub` proves it can see who references a symbol — so a rule like "every caller of `mcp_read_opts.describePcbOpts` must appear in `schema_readers`" would turn my honour-system comment into a real ratchet. Same shape as `fuzz_presence`'s "these modules must keep calling `std.testing.fuzz`", pointed at callers instead of modules.
- **good:** final whole-tree gate under `scripts/gate.sh`: 72 checks, 0 blocking, 2702 tests green, test wall 68.57 s; `zig build test-compile` clean. Both new guards were verified by deliberate sabotage (dropping `rough` from one schema, reverting one property's type) and each failed with the offending name, so they are known to bite rather than merely to pass.

## 2026-08-13 · codex · eda — export rf-switch-eval Gerbers and KiCad handoff
- **friction:** the filtered test runner prints a `failed command:` line even when the guarded test process exits 0; one mistyped filter also correctly failed as “nothing you asked for ran,” but the surrounding output made the first diagnosis ambiguous until I appended an explicit exit marker.
- **bug:** none found in Guardian; the new `add_zone` spec tag initially reported as unlinked until its matching `SPEC.md` bullet was added.
- **good:** the report-only complexity and boolean-condition findings named the exact new offenders, letting the implementation extract the text/zone emitters and keep the final whole-tree gate at 0 blocking checks.
- **wish:** include the process exit status and selected-test count in the default build summary’s final line so a successful guarded test cannot be mistaken for a failed command.

## 2026-08-13 · claude · eda — export a footprint pad's own rotation to KiCad
- **good:** `cognitive-complexity`'s per-item ratchet fired on exactly the right function and produced a better fix than the one I would have shipped. Adding one `if (cl.len >= 4)` to `emitKicadPad` took it to 31 against a frozen ceiling of 29, and the finding named the function and printed both numbers, so the size of the debt was unambiguous. The obvious response was to accept; instead I extracted the two things the new code touched — `readPadPos` for `(pos X Y [ROT])` and `emitPadAt` for the `(at …)` line — which dropped the function below its ceiling AND gave the pad-rotation convention a single doc-commented home instead of an inline comment buried in a 60-line parse loop. The ratchet did what it is for: it made "one more branch in the big function" cost more than the refactor.
- **good:** `test_reachability`-adjacent behaviour, via the counting test runner, found a genuinely orphaned test for me at zero cost. I ran `zig build --seed=1 test -Dtest-filter='footprint preview JSON separates'` expecting to confirm a file's tests ran, and got `0 match by name, 16 unnamed test block(s) run regardless` — proof that `src/serve/footprint_preview.zig` was not reachable from the aggregator and its one existing test had never executed. Without the "N selected by filter" line that run would have looked identical to a pass. Wiring the file in was a one-line fix and the suite went 2702 → 2704.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test … --listen=-` on a GREEN run (twelfth-plus report; logging as a vote). Cost this session was real: I saw the banner on my first focused run, could not tell whether my two new tests had failed, ran the test binary by hand to get `All 55 tests passed`, and then — because the banner ALSO appeared on a run where guardian was blocking — spent a further round trip stashing my whole change to prove the banner predated it. Two round trips to establish "this line means nothing". What settled it was `zig build … > log 2>&1; echo EXIT=$?` printing `EXIT=0`. One observation that might narrow the fix: on the run where a test genuinely leaked, the build printed the real detail (`58 pass (58 total); 2 leaks`, the leaking test's name, and `Build Summary: 17/19 steps succeeded`) and the banner sat alongside it; on clean runs the step output truncates to `+- run test w` and the banner is all that is left. So the banner is not the bug so much as the *absence* of a final "N/N tests passed, exit 0" line to outrank it.
- **wish:** a check for "two readers of one file format must agree on a field". This defect was four parsers of the same `(pad … (pos X Y ROT))` grammar where exactly one read the third token, so netlisp routed and DRC'd a rotated pad and then exported axis-aligned copper for it. Nothing in the gate could see that, because each reader is individually well-formed. My guard is a hand-written differential test that parses one fixture with both readers and asserts the field agrees — which works, but is a pattern I had to invent per site. Guardian already ratchets "these modules must keep calling `std.testing.fuzz`" (`fuzz_presence`); a `[[twin_field]]`-style rule naming two functions that must both mention a token, or simply a report of "form `pos` is read in 4 files with differing arity", would have surfaced this class before it shipped.
- **good:** final whole-tree gate under `scripts/gate.sh`: 72 checks, 0 blocking, 2704 tests green, test wall 66.67 s; `zig build test-compile` clean; the commit hook's own whole-tree run also 0 blocking.

## 2026-08-13 · claude · eda — /mcp-tools docs: render union JSON Schema types (blank Type cell fix)
- **good:** the counting test runner's zero-match guard earned its keep: `zig build test -Dtest-filter='union types'` reported "0 match by name" and FAILED instead of exiting 0 — which is how I discovered `src/serve/mcp_docs.zig` was never bridged into main.zig's test block (serve.zig only references it inside a route-registration body). Its three existing `// spec:`-tagged tests had never been collected by any test binary. One `_ = @import` line revived them; full suite went 2704 selected, green.
- **wish:** the spec check verifies bullet↔tag 1:1 but not that a tag-bearing test is ever *collected* — a `// spec:` tag on a test unreachable from the test root reads as coverage while being dead code. Cross-checking tags found in source against the runner's selected-test names (or the test root's import closure) would have flagged mcp_docs.zig's three dead tests the day they landed.

## 2026-08-13 · claude · eda — one canonical paint order for the three PCB renderers
- **good:** `test-no-conditional` fired twice on new tests ("more than one top-level loop") and both times the suggested fix was strictly better than what I wrote. The first was a stage-table test with a name-uniqueness loop followed by a review-rank-permutation loop; merging them into one `for (stages, 0..)` made the test read as a single statement about a single table instead of two half-statements. The finding names the loop that asserts nothing, which is exactly the one to fold in — I did not have to guess which of the two it meant.
- **good:** `dead-pub` paid for itself in one line. A refactor replaced the only caller of `pour.sideOfOuterLayer`, and the check named the file, the symbol and the three ways out. The right answer was "delete" — `sideOfSignal` already answered the same question for every layer, so what looked like an unrelated tidy-up was really the removal of a second rule for one fact. Without the check that function would have sat there indefinitely as an attractive nuisance for the next caller.
- **good:** `pub-api-surface`'s delta line ("1 new symbol(s), 0 changed, 0 removed — pure additions, safe to accept") made an intentional export decision reviewable in one glance, and `GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface` produced a 5-line `.guardian/pub-api.txt` diff I could read in the commit. Two accepts this session, both a few seconds.
- **friction:** the `spec` check's 1:1 bullet↔tag rule forced one behaviour into two bullets. "Footprint silk paints above copper" is one physical statement that needed two tests (a pixel test on the PNG renderer, a text test on the shipped viewer JS), and sharing one tag reported as `duplicate tag`. The message was clear and the fix was 30 seconds, but the SPEC now reads as two behaviours where the design has one. A `// spec:` tag that could name the same bullet from N tests — or a documented convention for splitting one behaviour across surfaces — would keep SPEC.md a list of behaviours rather than a list of tests.
- **friction:** `failed command: cd . && ./.zig-cache/o/<hash>/test … --listen=-` on a GREEN run again (this is now many sessions running; logging as another vote, and confirming the diagnosis in the entry above). Cost was two extra round trips at the start of the session before I settled on `; echo EXIT=$?` as the only trustworthy signal. Concretely: on a clean run the step output truncates to `+- run test w` and this banner is the last thing printed, so there is nothing to outrank it; on a failing run the real detail prints alongside it and the banner is harmless. A final `N/N tests passed, exit 0` line would end this.
- **good:** final whole-tree gate under `scripts/gate.sh`: 72 checks, 0 blocking, 4 report-only, 2721 tests green, test wall 70.25 s; `zig build test-compile` clean throughout.

## 2026-08-13 · claude · eda — fix custom-pad at-angle bake sense in convert/footprint
- **good:** the spec workflow shaped the change for the better. The task was "fix a sign bug", but writing the SPEC bullets first forced the three distinct behaviors apart (custom-pad bake sense, quarter-turn flatten, non-quarter preservation), and the 1:1 tag rule meant each got its own hand-computed test instead of one blob test. The commit-time whole-tree gate accepted all three bullets + tags in one pass, 0 blocking.
- **good:** diff-scoped per-file checks kept every focused iteration honest and cheap — `1/381 source file(s) in scope` on each filtered `zig build test`, with the whole-tree guarantee deferred to the commit hook, which is exactly the right split for a one-file fix.
- **friction:** another vote for the `failed command: … ./test … --listen=-` banner on a green run (the two entries above diagnose it). One new observation to add: the banner is intermittent for the SAME command, and a banner run still records the run step as SUCCESS — my immediate identical re-run was a silent cached no-op printing nothing at all, so the sequence read "failed, then nothing", which looks even more load-bearing than the banner alone. Cost: three probe builds plus one direct test-binary execution (`All 32 tests passed`) to re-establish that `echo exit=$?` (0) was the only signal that mattered. The fix wished for above — a final `N/N tests passed, exit 0` line that outranks the banner — would also dissolve the cached-no-op confusion, since the banner would never be the last word.
- **good:** final whole-tree gate under `scripts/gate.sh`: 72 checks, 0 blocking, 2703 tests green, test wall 67.37 s; `zig build test-compile` clean between the focused runs and the gate.

## 2026-08-13 · codex · eda — bounded Barracuda field-residual optimization
- **good:** `cognitive-complexity` caught `retryFieldCluster` at 26 against a 25 ceiling after one extra safety guard; extracting the attempt-eligibility predicate restored the gate and made the bounded preconditions explicit instead of ratcheting new debt.
- **good:** `pub-api-surface` showed one pure addition (`openNetsAmong`) with the complete signature and the exact snapshot-accept command; the resulting one-line snapshot diff was easy to audit.
- **friction:** filtered green tests repeatedly ended with `failed command: ... test ... --listen=-` despite exit 0, so every focused run required a follow-up exit-status poll before it was trustworthy. This task ran six filters while iterating; a final explicit success line would have removed that ambiguity.
- **good:** the final exact-commit preparation passed all 72 Guardian checks with 0 blockers, the full Debug suite in 85 seconds, and the ReleaseSafe build in 253 seconds (257 seconds concurrent wall).

## 2026-08-13 · Claude (Fable 5) · eda — port padRotOut diagonal-pad landing into import-kicad's emitBoardPad
- **good:** `pub-api-surface` caught the newly-pub `padRotOut` with a precise "1 new symbol, 0 changed, 0 removed — pure additions, safe to accept" delta plus the exact accept command; one `GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface zig build` ratified it and the snapshot committed with the change.
- **friction:** one more `failed command: … ./test … --listen=-` banner on a green run (cost: one redundant full-suite gate re-run). New fact for the triage of the entries above: the line is the pinned Zig toolchain's own — `lib/compiler/Maker/Step/Run.zig` records the child argv as the step's failed-command UNCONDITIONALLY before the pass/fail verdict exists (its comment says so) and only clears it on the success path, so any surviving echo is pre-verdict noise, not a verdict. Guardian can't fix Zig, but the wished-for final `guardian/test: PASS (N run, 0 failed)` line from the counting runner would outrank the banner in every transcript.
- **good:** spec bullets ↔ tagged tests 1:1 was frictionless for two bullets landed with their tests in one change; diff-scoped gate on a fresh worktree (2/381 files in scope) plus the prebuilt guardian-check kept every iteration fast, and the whole-tree gate finished 72 checks / 0 blocking with a 66.50 s test wall.

## 2026-08-13 · codex · eda — integrate Gerber/spec and field-router branches
- **good:** the post-rebase exact-commit gate validated the combined two-commit tree with all 72 checks green, full Debug tests in 85 seconds, and ReleaseSafe build in 243 seconds; the prepared candidate was reused by the merge hook and every deployment health probe passed.
## 2026-08-13 · Claude · guardian-zig — Wave C: three new relational checks (divergent-const, twin-referent, duplicate-json-key)

- **good:** self-hosting caught a real finding in the very first run of a new check. `divergent-const` fired on Guardian itself: `max_file_bytes` was 16 MiB in `cache.zig` and 4 MiB in `source_digest.zig` (with `walk.WalkOpts.max_file_bytes` a third value, 10 MiB). One name, three facts. Renaming two of them to `max_input_bytes` / `max_source_bytes` was the whole fix, and I would not have gone looking.
- **good:** running a new check against a SECOND project before trusting it is what made it usable. `duplicate-json-key` produced 19 findings on eda of which 4 were real; the 15 false positives were all one shape I had not modelled (mutually-exclusive branches: `if (v) |x| print(",\"voltage\":{d}") else writeAll(",\"voltage\":null")`, switch prongs, `return w.writeAll(...)` early-outs) plus one more (`std.mem.indexOf(u8, body, "\"dnp\":true")` — a key fragment in code that READS rather than writes). Two extra rules took it to 2 findings, both hand-verified. A precision-first check needs a corpus that is not the repo it ships in.
- **friction:** `failed command: ./.zig-cache/o/<hash>/test … --listen=-` on a GREEN run — but this session found a REPRODUCIBLE trigger, which the previous nine-plus reports did not name: it appears when `zig build test` is **piped** (`| tail`, `| grep`), and never when its output is redirected to a file or run bare. `mutation.runner.test.superviseArgv kills the whole process group on timeout` does exactly that — `kill(-pgid)` — and under a pipeline the test binary shares its process group with `tail`, so the signal reaches the pipeline and the build system reports the step as failed while `zig build` still exits 0. Cost here: three extra suite runs before I stopped believing the banner. Suggested fix in Guardian's own suite: have `superviseArgv`'s test put its child in a NEW process group (`setpgid`) before killing, so the blast radius is the child, not whatever shell plumbing the developer used.
- **friction:** `zig build mutate` on a branch with ~1300 changed lines took **14m29s** for 8 sampled mutants, because each mutant rebuilds a ReleaseSafe `guardian-check` (~90 s of LLVM) before running a 0.5 s test suite. The mutation signal was worth it — all three survivors named real assertion gaps — but the cost is ~99% compile. A `-Doptimize=debug` child build for the mutation tier would trade a slower test run (0.5 s → maybe 2 s) for ~85 s per mutant.
- **wish:** the fast mutation tier samples 8 mutants from 263 candidates by hash, so re-running after killing survivors re-rolls a different 8 rather than re-checking the three I just fixed. I hand-spliced each of the three mutations back into the source to prove my new tests fail — which worked, and is what I would want a `mutate --recheck` (replay `last-mutate.jsonl`'s survivors only) to do for me.

## 2026-08-13 · Claude (Fable 5) · eda — merge + deploy of the padRotOut diagonal-pad port
- **good:** rebasing the branch over a main that had itself just accepted a pub-api snapshot produced NO conflict in `.guardian/pub-api.txt` — the guardian merge-file driver reported "merged 3187 pub_api row(s)" and both sides' additions coexisted, where a textual merge of a sorted snapshot would almost certainly have conflicted. This is exactly the multi-session workflow (two branches each ratifying their own pub additions) and it was invisible-smooth.
- **good:** both prepare-release runs (pre- and post-rebase) went whole-tree gate → tests → build with 0 blocking on the first try; the second run's gate re-verified the rebased tree rather than trusting the stale candidate, and the merge then deployed by candidate adoption in ~1 s.

## 2026-08-13 · claude-fable · eda + guardian-zig — layer-standardization + relational-checks campaign (orchestration level)

- good: the concept check's first live run found a tenth rail classifier grep is blind to (editor.js's NUL bytes make grep treat it as binary) — the lexical scanner out-performed four grep-based audit passes on day one.
- good: the FEEDBACK loop closed inside one campaign: the eda activation documented two concept-check limitations (quoted literals, per-rule files) at its skip site, and Wave B shipped both fixes the same day, unblocking the drc-kind-ids declaration exactly as written.
- friction: merging guardian main + rebuilding zig-out/bin/guardian-check opens a selfcheck-stale window where EVERY eda build fails until the rebuild lands (~90 s). With several eda sessions live this needs manual traffic control (wait for a no-gates window). wish: an atomic swap helper (build to temp, rename binary + stamp together) or a selfcheck grace that tolerates sources newer than binary by < N seconds while a rebuild runs.
- wish: subagents repeatedly ended turns on orphaned background gate waits, reporting "gate running" for processes their stop had killed — three of eight needed a resume-and-rerun. A `gate.sh --status` (holder pid + last exit) would make "is the gate actually running" a one-command answer instead of pgrep archaeology.

## 2026-08-13 · claude · eda — release the pad at-angle fix (merge race night)
- **good:** `guardian-selfcheck` failed closed at exactly the right moment. Between my first and second `prepare-release.sh` runs, guardian-zig itself merged the Wave C relational checks (23:51–23:52), and the eda gate refused the now-stale prebuilt binary with both hashes and the one-line remedy (`binary b0328980287e, source 3c1e148859e1 — run zig build in ../../canopy/guardian-zig`). The rebuild took a minute, the re-run passed under the new 75-check gate, and at no point could a release verify against an outdated checker. Cost was one wasted ~4 min prepare-release cycle, priced correctly for what it prevented.
- **good:** three prepare-release runs in one evening while two other sessions merged into main and into guardian-zig; the tree-keyed candidates never mis-adopted — my superseded candidates (00e13bdf, b25693a7) just sat unused, and the deploy that actually shipped adopted the winning branch's own verified tree in ~1 s (`adopted verified candidate for identical tree … built for 9f487d07`), health OK.

## 2026-08-14 · claude · eda — merge/deploy of the mcp-docs union-type fix
- **good:** guardian-selfcheck failed my prepare-release fast (~3 s in) when guardian-zig merged new checks (72→75) between my two prepare runs, and the message named the exact fix (`zig build` in the guardian checkout). Cost was one retry cycle + a 2-min rebuild — cheap for the guarantee that a stale gate binary never verifies a release.
- **wish:** a mid-flight guardian-zig merge invalidates every downstream project's prepare-release until someone rebuilds the prebuilt binary by hand. A `GUARDIAN_PREBUILT` auto-rebuild mode (selfcheck detects staleness → rebuilds once, ReleaseSafe, then proceeds) would turn that cross-repo coupling into a ~2-min pause instead of a failed gate + manual step.

## 2026-08-14 · claude-fable · eda — declare drc-kind-ids + freeze the three relational checks
- **good:** the Wave B/C fixes landed exactly as the eda skip-note requested, and both were verifiable from the outside without reading Guardian's source. Quoted literals: declaring `literals = ["\"track_track\""]` scored **1** offender, where an unresolved escape would have scored 0 and a bare match 49 — the count itself is a three-way proof the escape resolved, so no trust was required. Per-rule `files`: a throwaway `files = ["src/*.js"]` rule whose literal also appears in five Zig files reported the `.js` file alone. Both probes took under a minute each. Shipping the fix with the failure modes distinguishable by count is what made that cheap.
- **good:** `divergent-const` in units mode was high-signal on first contact in eda: 10 names, ~0 false positives, and the standouts were real board-correctness bugs, not style — `silk_stroke_mm` 0.15 in two silkscreen writers and 0.12 in a third (two silkscreens from one board), `max_footprint_bytes` 1 MiB in four readers and 256 KiB in two (a footprint that loads in the editor and fails in the preview). The name-keyed baseline is the right grain: a third file joining an existing disagreement lands on the existing row rather than arriving as new debt.
- **friction:** declaring a NEW `[[concept]]` rule is refused by `deny_growth` ("acceptance refused because the configured deny_growth policy would grow recorded debt"), because the ratchet cannot distinguish a newly-DECLARED rule's rows from a newly-APPEARED hand-copy. The workaround is a deliberate two-step (drop `concept` from `deny_growth`, accept, put it back), which is defensible friction — but it is undiscoverable at the point of failure. Cost here: one failed accept plus the reasoning to be sure lifting the ratchet was legitimate rather than me routing around a real finding. **wish:** either name the two-step in the refusal message, or add `guardian-check accept <check> . --new-rule` that permits growth attributable to a rule name absent from the current baseline.
- **friction (minor):** `guardian-check accept a b c .` silently accepts only the FIRST named check — it printed three run blocks and "verified 1 named check(s)", and only `divergent-const.txt` was written. I only noticed because I listed the baseline files afterward. Accepting them one at a time worked. **wish:** either accept the list, or reject a multi-check invocation with a usage error rather than half-doing it.
- **good:** the "grandfather trap" is real but the docs warned me and the warning held: with no baseline file the first run of `divergent-const` / `twin-referent` / `duplicate-json-key` reported 10 / 8 / 2 findings as *grandfathered* and exited **0**, and `--verbose` still suppressed the detail. Writing a header-only baseline (`# guardian-snapshot v3`) first turned all three into ordinary NEW violations with full per-finding detail — a clean way to see what you are about to freeze. **wish:** a `--show-grandfathered` (or having `--verbose` simply print them) would remove that dance; the information is clearly in hand at that point.
...
## 2026-08-15 · codex · eda — validate a custom float-pair compiler ABI
- **good:** The exact-commit release boundary exposed a broken compiler experiment that had passed the compiler's x86 stress and native ReleaseSafe case suites: Guardian itself was green, but the complete EDA Debug run failed 105 tests and crashed 3 of 2,927. After repairing the compiler's callee-save encoding, the same gate passed all 2,927 tests and the concurrent ReleaseSafe build before publishing a fingerprinted candidate.
- **good:** Compiler SHA-256 cache isolation forced both custom compilers to produce distinct test/build artifacts despite reporting the same pinned Zig version, preventing the corrected run from adopting the invalid experiment's output.
- **friction:** The corrected 19-test filtered run printed `failed command: ...` immediately after `guardian/test: PASS`; as in prior sessions, only the eventual outer exit status established that the run was green.

## 2026-08-15 · codex · eda — require full-width trace junctions
...
## 2026-08-17 · claude · eda — fix the early call-argument-death compiler miscompile
- **good:** The `-Dtest-filter="stuck diagnosis"` router discriminator did exactly its job again: it reproduced the deterministic `router.zig:4401` crash under the experimental compiler in ~1s of test wall, then flipped to 17/17 in both Debug and `-Dtest-opt=safe` once the x86 backend bug was fixed. Cheap enough to run after every compiler rebuild.
- **good:** The full suite was the check that mattered at the end: 2,943 tests passed with both the repaired compiler and the retained control on the same checkout, which is what let me claim the fix is neutral rather than merely un-crashing.
...
## 2026-08-17 · claude · eda — barracuda reference-free autoroute iteration loop
- **good:** the per-item ratchet caught real design pressure rather than noise: `type-size` fired the moment `route_close.Options` reached 8 fields, and the fix it forced (folding the pass-scoped clock + memo into one `Pass` struct) is the shape I should have written first. Same for `bool-ops-per-condition` on a 5-clause float comparison — extracting `samePoint` read better afterwards.
- **good:** `GUARDIAN_UPDATE_SNAPSHOT=pub-api-surface zig build` is exactly the right granularity for a change that genuinely adds public API; the diff in `.guardian/pub-api.txt` was small enough to eyeball as a review artifact.
- **friction:** the `spec` check reports `unverified: <bullet>` but not which FILE it expected the tagged test in. On a bullet added under `serve/route-plan` I had a test in `route_close.zig`, which is a different section, and the message could not distinguish "no test" from "test in the wrong section". A `(section owns src/serve/route_plan.zig)` hint in the finding would have saved a build cycle.
- **friction:** `test-no-conditional` fires on a `while` loop at the top level of a test BODY, including a loop whose only job is building a fixture (refuse 8 distinct hops so a per-net cutoff can be observed). Extracting it to a helper satisfied the check but the helper exists only to hide a loop from the checker — the assertion structure did not change. A carve-out for loops with no `try testing.*` inside would keep the check aimed at branching assertions.
- **wish:** each gated `zig build` on this tree costs ~35-50 s wall even when only one function changed, and roughly a third of that is guardian's own analysis of the 21-file diff scope. A `--since-last-green` mode that only re-analyzes files whose content hash moved since the last cached green run would compound over an iteration loop like this one (14 build/measure cycles in a session).

## 2026-08-17 · claude · eda — per-gap unblock transactions + remedy fix
- **good:** `change-classification` did exactly the right thing twice: it blocked a commit whose only new lines were debug `std.log.debug` tracing, which made me decide deliberately whether the tracing was worth a SPEC bullet (it was not — I reverted it after it had served its diagnostic purpose). A checker that makes "is this worth keeping?" an explicit decision rather than a default-yes is worth its friction.
- **good:** the `spec` check's `unverified: <bullet>` naming caught a bullet I had added under one section with its tagged test in another file's section. Second session in a row this check paid for itself.
