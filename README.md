...
# twin-drift: two same-named functions in different files whose copied bodies
# have drifted apart. `ignore` names the functions a project implements once
# per file as an INTERFACE (they are meant to look alike); the thresholds
# control which pairs are worth comparing at all.
[twin_drift]
min_statements = 8       # normalised body lines before a fn is compared
min_similarity = 0.6     # 2*|LCS| / (|A| + |B|), exclusive of 0 and 1
...
| `[twin_drift]` | `min_statements` (>= 2, default 8), `min_similarity` (0 < x < 1, default 0.6), `report_identical`, `ignore`, `max_lines` |
