...
| **projection-completeness** *(configured)* | A struct literal that **projects** a declared bundle of fields — the "rebuild this value out of the parts I happen to hold" shape — while omitting part of the bundle. The compiler cannot see it: every field of such a struct DEFAULTS, so an omitted field is not an error, it is a silent empty. Measured in eda (2026-09, from that repo's own fix commits): `pour.Copper` gained a defaulted `arcs` field and five literals that rebuilt a Copper from a routed result kept projecting the old bundle, so the connectivity oracle they feed read a net joined only by an arc as an OPEN; ten sites were fixed by hand over three follow-up commits and two were still wrong after that. A `[[projection]]` entry names the `type` and the `fields` that must travel together; a literal setting SOME of them but not ALL of them is the finding, and a literal setting NONE of them is not a projection of that bundle at all. `type` matches the TAIL of the literal's type path at a segment boundary — bare `Copper` covers `Copper{`, `pour.Copper{` and `routed_copper.Copper{`, qualified `routed_copper.Copper` covers only the last, which is what a repo with several same-named types needs. An anonymous `.{ … }` literal has no path to match, so it is judged only when it sets `anonymous_min_fields` (default 2) of the declared set AND sets nothing outside `fields` + `optional`; that half is weaker by construction (on eda it reported 98 anonymous literals to the typed half's 4, most of them unrelated structs whose vocabulary is a subset), so `anonymous_min_fields = 0` turns it off. `test` blocks are skipped, and `// projection-ok: <why>` on the literal's line, on a comment line directly above it, or on the enclosing statement's line is the in-source exemption — the omission gets a reason instead of a baseline row. Keyed `<name>|<file>|<fn>|<sorted fields set>`; no entries = a trivial pass |
| **twin-drift** | Two functions in DIFFERENT files whose copied bodies have stopped agreeing. Measured in eda (audit of 155 fix commits, 2026-09): "two hand-written copies of one rule drifted" is the cause behind **25** of them. The live case is `buildNetClassOverrides`, duplicated in `drc_session.zig` and `wasm_drc.zig` — the first file's header says the JSON board parser was copied on purpose to stay under the size cap, then the wasm copy gained `.class`, `.power_branch_width`, `.keepout_mm` and `.keepout_escape_mm` and the session copy gained none of them, so the session DRC now runs with no keepout rule. The two share no type, no call and no file, so nothing in the compiler can see it and each copy's own tests keep passing. Similarity is `2*|LCS| / (|A| + |B|)` over normalised body lines (comments and blanks dropped, internal whitespace collapsed, one entry per source line), reported as "share N% of their body" — the motivating pair measures **81%**. **IDENTICAL bodies are NOT reported**: byte-identical twins are duplication debt, and listing them buries the pair that is actively wrong (the same two files hold four of them); `report_identical = true` asks for that inventory. A `mirrors X` / `same as Y` claim does NOT exempt a pair — a declared mirror that drifted is the worst case — but the message then says `(declared mirror)`. Two populations are excluded by construction: a fn inside a `test` block, and a private fn reachable only from `test` blocks in its own file (directly or through another such helper) — skipping those per-file fixtures took eda from 203 findings to 69. Bodies under `min_statements` (default 8) are scaffolding and never compared; a body over `max_lines` (default 400) is counted and skipped. **Pairing is name-agnostic (v2)**: each body is re-tokenised with Zig's own tokenizer (every string/char literal collapsed to `$str`, every number to `$num`), becomes the multiset of its 3-gram token shingles, and a pair is PROPOSED when the tf-idf cosine over those shingles reaches `pair_similarity` (default 0.5) — then judged by the LCS above, unchanged. An inverted index accumulates the cosines sparsely, through shingles held by at most 96 bodies, so two functions overlapping only in boilerplate are never compared and a 510k-line whole-tree pass stays at **0.49 s** (v1's name-grouped pass: 0.26 s). Measured on eda, v2 keeps 56 of v1's 69 pairs and adds 70, every one of them a copy under a different name (`shapeOfPoly`/`shapeFromWorldPoly`, `isSafeLibName`/`isSafeFootprint`); the scaffolding-only pairs v1 misreported score 0.44 and 0.33 against the 0.5 floor. Exempt with `// twin-drift-ok: <reason>` above either copy, `[twin_drift] ignore` for a name a project implements once per file as an interface, or `[[allow]]`. **The idf table is FROZEN, and what BLOCKS is narrowed.** A LIVE `idf` moves whenever a body is added or removed ANYWHERE, so a pair just under the floor crosses it with no edit to either of its own files — measured in eda (2026-09-02), merging one of five reconciliation branches removed 40 bodies and the next branch's rebased tree reported `module_policy.stripUpper` / `pin_roles.normalizeIdent` (90% LCS) as a NEW blocking row in two files no branch had touched, leaving main red until a later branch's baseline happened to carry it. The pairs were real drift, so the proposal and the LCS judgement are unchanged and the *verdict* is narrowed instead: a pair already in the baseline is LIVE, an unrecorded pair BLOCKS only when the change touched one of its two files (the same diff scope everything else uses — `--against` / `GUARDIAN_AGAINST`, else the merge base with main/master, working tree plus index), and every other unrecorded pair is **SURFACED** — advisory, not recorded by the run, collapsed to one line (`N pair(s) surfaced by corpus shift, none in files this change touched`) that `--summary` may not bury, expanded per pair by `--verbose`, bucketed by `--list` between NEW and LIVE, and untouched under `--dry-run`. With no resolvable base nothing can be proven untouched, so every unrecorded pair blocks and the run says so. `guardian-check accept twin-drift .` records the surfaced pairs, and does so even under `[baseline] deny_growth` — but only for pairs it proved neither file changed against the base. **The other half is the frozen df table** (`.guardian/twin-drift-df.txt`): the document count and every shingle's frequency as they stood at the last accept, read in place of the tree in front of the check. Both the weight and the `2 <= df <= 96` proposal gate read the frozen value — the gate too, because a boilerplate 3-gram drifting across 96 adds real mass to an untouched pair's cosine — so two bodies that did not change score bit-identically however the rest of the tree moved. Replayed over the study's six eda states (`docs/twin-drift-scoring-study-2026-09-02.md`), a table frozen at the first reproduces the frozen-idf reference exactly five merges later: 125/85/49/31/14/0 pairs, and `stripUpper`/`normalizeIdent` never appears. A shingle the table has never seen falls back to its live frequency, so code added since the freeze is still paired normally. With no table the check is byte-identical to what it was before one existed, and only an accept naming twin-drift (or `GUARDIAN_UPDATE_SNAPSHOT=twin-drift`) writes one — no ordinary run, `--gate`, `--dry-run` or `--list` ever creates or rewrites it. The whole vocabulary is stored, df=1 rows included, because a shingle the table lacks falls back to live and a 3-gram that was unique at freeze time and is shared now IS the corpus shift: on eda a df>=2-only table re-admitted the very pair this removes (2.5 MB / 264,931 rows with the truncated base-36 key and the implicit df=1, against 5.9 MB raw). An unreadable table warns and scores live rather than degrading in silence; `--list` heads its output with the table's coverage and its frozen N against the live one; `merge-file` keeps OURS whole on a conflict and marks the file for regeneration, because the rows are one measurement of one corpus — and a table carrying that marker is still USED (one branch's freeze is a real measurement) while every run says so once, on the alert tier, until an accept re-measures it on the merged tree. Freezing removes drift caused by Guardian's own scoring only — the narrowed verdict above is what makes each refresh safe to perform. One violation per pair, keyed `<nameA>|<fileA>|<nameB>|<fileB>` with the sides ordered by path, so editing either copy further moves the percentage without re-keying the row; the differing lines ride the advisory channel |
...
**Freeze a baseline against growth.** For the checks whose debt should only ever shrink — the 1:1 spec map is the canonical case — list them in `[baseline] deny_growth`. What counts as growth differs by flavor, because the two files measure different things:

* a **per-item ratchet** (baseline v2, the threshold checks) refuses a refresh that would *raise* a recorded value **or add a key** — each key is its own ceiling, so either move is a regression;
* an **identity baseline** (v3, every other check) refuses a refresh that would leave the file **larger than it found it**. A row is one violation, so the count IS the debt: a reconciliation that resolves eighteen rows and records two lands, and a swap is not growth. (Documented as one rule until eda hit the difference in practice on 2026-09-02.) One narrow exception: a check may mark an addition *growth-exempt* when it has PROVEN the row is pre-existing debt its change only made visible — today only `twin-drift`, for a pair surfaced in files the change never touched. Those additions are discounted from the comparison; everything else still refuses.

Either way the refusal names the offending keys instead of ratifying the growth:
...
# A field bundle that must travel together, enforced by the
# `projection-completeness` check. Use one when a struct is routinely REBUILT
# out of the parts a caller happens to hold — every field defaults, so adding a
# field leaves every existing literal compiling and silently empty. name:
# kebab-case and unique; it leads the baseline key. type: the struct's type
# name, matched against the TAIL of a literal's type path at a segment boundary
# (bare `Copper` covers `pour.Copper{` too; qualify it when the repo holds
# several same-named types). fields: 2+ names that must be set together.
# optional: the rest of the type's fields — never a violation, but it completes
# the vocabulary anonymous matching checks against. anonymous_min_fields: how
# many declared fields a typeless `.{ … }` literal must set before the rule
# claims it (default 2; 0 turns anonymous matching off, which is what field
# names as common as `tracks`/`vias` want). allow: path globs. reason: required,
# and it closes every violation.
[[projection]]
name = "routed-copper"
type = "routed_copper.Copper"
fields = ["tracks", "vias", "arcs", "rf_paths"]
optional = ["zones", "silk_keepouts"]
anonymous_min_fields = 0
allow = ["src/placement/pour.zig"]
reason = "connectivity oracles carve every copper kind; a projection that drops one lies about opens"
...
# shadowed-const: one [[shadow]] rule per constant whose value must not be
# re-spelled as a bare literal elsewhere. `const` is the mirror-of referent
# spelling; `files`/`ignore` scope the scan; `reason` ends every violation.
[[shadow]]
const = "src/export_fab.zig.auto_outline_margin_mm"
files = ["src/placement/*.zig"]
ignore = ["src/placement/vendor*"]
reason = "the pour raster must follow the same Edge.Cuts outline"

# The auto sweep is a MEASUREMENT tier, not a gate: it hunts every unit-suffixed
# file-scope const's value, so it needs noise controls the declared rules above
# deliberately ignore.
[shadowed_const]
mode = "declared"
ignore_values = ["0", "1", "-1", "2", "0.5", "10", "100", "1000"]
min_float_digits = 2
min_int_digits = 3

# twin-drift: two functions in different files whose copied bodies have
# drifted apart. Pairing is name-agnostic — a tf-idf cosine over 3-gram token
# shingles proposes the pair (`pair_similarity`) and the line-level LCS judges
# it (`min_similarity`). `ignore` names the functions a project implements once
# per file as an INTERFACE: those are meant to look alike, and a shared body is
# exactly why the cosine cannot tell them from a copy.
[twin_drift]
min_statements = 8       # normalised body lines before a fn is compared
pair_similarity = 0.5    # tf-idf cosine to PROPOSE a pair, exclusive of 0 and 1
min_similarity = 0.6     # 2*|LCS| / (|A| + |B|) to REPORT it, same range
report_identical = false # byte-identical twins are debt, not drift
max_lines = 400          # longer bodies are counted and skipped
ignore = ["run", "deinit"]
...
| `[[projection]]` | `name` (required, kebab-case, unique), `type` (required), `fields` (required, 2+ entries), `optional`, `anonymous_min_fields` (0 = off, else 2..`fields.len`; default 2), `allow`, `reason` (**required**) |
...
| `[shadowed_const]` | `mode` (`"declared"` (default) \| `"auto"`), `ignore_values`, `min_float_digits`, `min_int_digits` |
| `[twin_referent]` | `ignore` |
| `[twin_drift]` | `min_statements` (>= 2, default 8), `pair_similarity` (0 < x < 1, default 0.5), `min_similarity` (0 < x < 1, default 0.6), `report_identical`, `ignore`, `max_lines` |
...
- **The `run-all:` verdict line** closes *every* exit path — green, blocking,
  ...
  ```
  run-all: 83 check(s) passed
  run-all: 83 checks — 0 blocking, 3 report-only
