...
fn tripDir(a: Allocator, dir: []const u8, files: []const []const u8) !void {
    std.fs.cwd().deleteTree(dir) catch |e| std.log.warn("test setup {s}: {s}", .{ dir, @errorName(e) });
    try std.fs.cwd().makePath(dir);
    for (files) |f| {
        const path = try std.fmt.allocPrint(a, "{s}/{s}", .{ dir, f });
        try std.fs.cwd().makePath(std.fs.path.dirname(path) orelse dir);
        (try std.fs.cwd().createFile(path, .{})).close();
    }
}
