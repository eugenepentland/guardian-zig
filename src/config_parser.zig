...
const ArrayKind = enum { none, boundary, allow, ban, concept, twin, external };
...
fn validateValue(
    ...
    if (st.array_kind == .concept) {
        try validateTableName(allocator, "concept", kv, line_no, diag);
        try validateLiteralsFrom(allocator, kv, line_no, diag);
    }
    if (st.array_kind == .twin) try validateTableName(allocator, "twin", kv, line_no, diag);
...
/// Rejects a `[[concept]]` / `[[twin]]` `name` that is not kebab-case. The name
/// is what every violation says out loud and — as the tail of the baseline key —
/// what a `.guardian/` diff is read in, so it is an identifier a reader has to
/// live with, not free-form prose. `reason` is where prose belongs.
fn validateTableName(
    allocator: Allocator,
    table: []const u8,
    kv: KeyVal,
    line_no: u32,
