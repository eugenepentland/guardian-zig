...
pub const all: []const Command = &.{
    ...
    .{ .name = "naming", .summary = "Enforce Zig naming conventions (PascalCase types, camelCase fns)", .needs_ast = .yes, .run = check_naming.run },
    .{ .name = "function-size", .summary = "Cap function parameter count", .needs_ast = .yes, .run = check_function_size.run },
    .{ .name = "doc-comments", .summary = "Require /// doc comments on every public fn/type", .needs_ast = .yes, .run = check_doc_comments.run },
    ...
    .{ .name = "dead-pub", .summary = "Flag unused public declarations", .needs_ast = .yes, .run = check_dead_pub.run },
    ...
    .{ .name = "stub-body-ban", .summary = "Reject obvious stub function bodies (return undefined, placeholder panics, unreachable in value fns)", .needs_ast = .yes, .run = check_stub_body_ban.run },
    .{ .name = "doc-quality", .summary = "Reject empty or stub /// doc comments on public declarations", .needs_ast = .yes, .run = check_doc_quality.run },
    .{ .name = "comptime-quota", .summary = "Track @setEvalBranchQuota call count and max value via snapshot", .run = check_comptime_quota.run },
    .{ .name = "type-size", .summary = "Cap fields per pub struct/enum/union/opaque", .needs_ast = .yes, .run = check_type_size.run },
    .{ .name = "function-length", .summary = "Cap source lines per fn decl", .needs_ast = .yes, .run = check_function_length.run },
    .{ .name = "nesting-depth", .summary = "Cap brace-nesting depth inside fn bodies", .needs_ast = .yes, .run = check_nesting_depth.run },
    .{ .name = "test-coverage", .summary = "Require every pub fn to be referenced from a test block (opt-in)", .needs_ast = .yes, .run = check_test_coverage.run },
    ...
    .{ .name = "init-hygiene", .summary = "Reject init bodies with loops, conditionals, or switch statements", .needs_ast = .yes, .run = check_init_hygiene.run },
    ...
    .{ .name = "errdefer-in-init", .summary = "Require errdefer between multiple try calls inside init", .needs_ast = .yes, .run = check_errdefer_in_init.run },
    ...
    .{ .name = "returns-per-function", .summary = "Cap return statements per function body", .needs_ast = .yes, .run = check_returns_per_function.run },
    .{ .name = "line-length", .summary = "Cap source line length", .run = check_line_length.run },
    .{ .name = "vague-name-blacklist", .summary = "Reject vague identifier names on public declarations", .needs_ast = .yes, .run = check_vague_name_blacklist.run },
